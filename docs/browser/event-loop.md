...
═══════════════════════════════════════════════════════
  ...
  ┌──────────────────▼──────────────────────────┐
  │  3. 是否需要渲染？（由浏览器判断）              │
  │     是 → rAF 回调 → Style → Layout → Paint → Composite → rIC │
  │     否 → 进入下一轮事件循环                    │
  └──────────────────┬──────────────────────────┘
                     │
                     └──→ 回到步骤 1
...
═══════════════════════════════════════
  ...
  核心规则：所有微任务进入同一个 FIFO 队列
  按入队顺序严格执行，无类型优先级区分

  常见微任务类型：
  • queueMicrotask(callback)  → 直接入队
  • Promise.then/catch/finally → 状态变更时入队
  • MutationObserver          → DOM 变动时入队
  • async/await               → await 后的代码等价于 .then 回调

  ✅ ECMAScript 规范保证：所有现代浏览器行为一致
  ✅ 不存在"不同类型微任务优先级不同"的情况
  ✅ 唯一决定执行顺序的因素：入队时间
```
...
═══════════════════════════════════════════════════════
  ...
  |← 宏任务+微任务 →|← rAF →|← Style/Layout →|← Paint/Composite →|← idle →|
  |                 |        |                 |                    |         |
  0ms               5ms      8ms              12ms                15ms    16.67ms

  • requestAnimationFrame: 在 Style 计算和 Layout 之前执行
  • requestIdleCallback:   在帧末尾空闲时间执行（不保证每帧都有）
  • 注意：实际时间分配取决于当前帧的工作量
```
...
A: 两者属于不同的调度机制，不能简单比较"谁先"。`setTimeout` 是宏任务，在任务队列中调度；`rAF` 在渲染阶段执行（宏任务和微任务之后、Layout/Paint之前）。在同一帧中，通常是：宏任务（setTimeout）→ 微任务 → rAF → 渲染。但如果 setTimeout 的延迟导致它在下一帧才执行，则 rAF 可能先于 setTimeout。实际执行顺序取决于它们被调度的时机。
