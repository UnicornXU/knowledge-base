...
| 队头阻塞   | 有       | 有                | 应用层无，TCP 层仍有 | 无         |
| 头部压缩   | 无       | 无                | HPACK                | QPACK      |
| 服务器推送 | 不支持   | 不支持            | 支持                 | 支持       |
| 协议层     | TCP      | TCP               | TCP                  | UDP (QUIC) |

> **关于 HTTP/2 的队头阻塞**：HTTP/2 通过多路复用解决了**应用层**队头阻塞（多个请求可在同一连接上并行传输），但底层仍使用 TCP。当 TCP 发生丢包时，所有流都会被阻塞等待重传，这就是 **TCP 层队头阻塞**。HTTP/3 通过基于 UDP 的 QUIC 协议彻底解决了这一问题——每个流独立，一个流的丢包不影响其他流。
